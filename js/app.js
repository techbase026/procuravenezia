...
const USERS = [
  { email: "pm@rpserver.it", password: "PM123!", role: "pm", label: "Pubblico Ministero" },
  { email: "magistrato@rpserver.it", password: "Magistrato123!", role: "magistrato", label: "Magistrato" },
  { email: "fdo@rpserver.it", password: "FDO123!", role: "fdo", label: "Forze dell'Ordine" },
  { email: "admin@rpserver.it", password: "Admin123!", role: "pm", label: "Pubblico Ministero" }
];
...
function injectInstitutionalMasthead() {
  ...
  if (document.querySelector(".institutional-header")) return;
  ...
  header.innerHTML = `
    <div class="gov-topline">Governo Italiano</div>
    <div class="gov-mainline">
      <div class="gov-mainline-inner">
        <div class="gov-brand">
          <div class="ministerial-mark" aria-hidden="true">PR</div>
          <h2>Ministero della Giustizia - Procura RP</h2>
        </div>
        <div class="gov-search">
          <input type="search" placeholder="Cerca nei registri..." aria-label="Cerca">
        </div>
      </div>
    </div>
    <nav class="gov-navline" aria-label="Navigazione istituzionale">
      <span>Portale Magistratura e Procura</span>
    </nav>
  `;
...
function setupSearchFilter(tableBodyId, searchId) {
  ...
  const apply = () => {
    ...
    rows.forEach((row) => {
      ...
      const match = query.length === 0 || rowSearch.includes(query);
...
function renderDashboard() {
  ...
  if (kpi) {
    ...
    kpi.innerHTML = `
      <article class="kpi-card"><p>Fascicoli attivi</p><h3>${activeCases}</h3><span>Esclusi i procedimenti chiusi</span></article>
      <article class="kpi-card"><p>Mandati aperti</p><h3>${activeWarrants}</h3><span>Attivi e in lavorazione</span></article>
      <article class="kpi-card"><p>Sentenze definitive</p><h3>${finalSentences}</h3><span>Aggiornate dal registro</span></article>
      <article class="kpi-card"><p>Reparti operativi</p><h3>${departments.filter((d) => d.status === "operational").length}</h3><span>Su ${departments.length} unita totali</span></article>
    `;
  }
...
function actionCell(section, editAttr, deleteAttr, id) {
  if (!canWriteSection(section)) return '<span class="read-only-tag">Sola lettura</span>';
  return `<button class="action-btn edit-btn" type="button" ${editAttr}="${id}">Modifica</button><button class="action-btn delete-btn" type="button" ${deleteAttr}="${id}">Elimina</button>`;
}
...
function renderCasesTable(data) {
  ...
  body.innerHTML = [...data].sort((a, b) => new Date(b.createdAt) - new Date(a.createdAt)).map((item) => `
    <tr data-search="${`${item.id} ${item.title} ${item.prosecutor}`.toLowerCase()}">
      ...
      <td>${actionCell("fascicoli", "data-edit-case", "data-delete-case", item.id)}</td>
    </tr>`).join("");
}
...
function renderWarrantsTable(data) {
  ...
  if (!body) return;
  body.innerHTML = [...data].sort((a, b) => new Date(b.createdAt) - new Date(a.createdAt)).map((item) => `
    <tr data-search="${`${item.id} ${item.caseId} ${item.type} ${item.signer}`.toLowerCase()}">
      <td>${item.id}</td><td>${item.caseId}</td><td>${item.type}</td><td>${item.signer}</td><td><span class="badge ${item.status}">${warrantStateLabel(item.status)}</span></td>
      <td>${actionCell("mandati", "data-edit-warrant", "data-delete-warrant", item.id)}</td>
    </tr>`).join("");
}
...
function renderSentencesTable(data) {
  ...
  if (!body) return;
  body.innerHTML = [...data].sort((a, b) => new Date(b.createdAt) - new Date(a.createdAt)).map((item) => `
    <tr data-search="${`${item.id} ${item.caseId} ${item.judge} ${item.outcome}`.toLowerCase()}">
      <td>${item.id}</td><td>${item.caseId}</td><td>${item.outcome}</td><td>${item.judge}</td><td>${item.penalty}</td><td><span class="badge ${item.status}">${sentenceStateLabel(item.status)}</span></td>
      <td>${actionCell("sentenze", "data-edit-sentence", "data-delete-sentence", item.id)}</td>
    </tr>`).join("");
}
...
function renderDepartmentsTable(data) {
  ...
  if (!body) return;
  body.innerHTML = [...data].sort((a, b) => new Date(b.createdAt) - new Date(a.createdAt)).map((item) => `
    <tr data-search="${`${item.id} ${item.name} ${item.chief}`.toLowerCase()}">
      ...
      <td>${actionCell("reparti", "data-edit-department", "data-delete-department", item.id)}</td>
    </tr>`).join("");
}
...
function renderPeopleTable(data) {
  ...
  body.innerHTML = [...data].sort((a, b) => new Date(b.createdAt) - new Date(a.createdAt)).map((item) => `
    <tr data-search="${`${item.id} ${item.name} ${item.role} ${item.caseId}`.toLowerCase()}">
      <td>${item.id}</td><td>${item.name}</td><td>${item.role}</td><td>${item.caseId}</td>
      <td>${actionCell("persone", "data-edit-person", "data-delete-person", item.id)}</td>
    </tr>`).join("");
}
...
function renderHearingsTable(data) {
  ...
  if (!body) return;
  body.innerHTML = [...data].sort((a, b) => new Date(b.createdAt) - new Date(a.createdAt)).map((item) => `
    <tr data-search="${`${item.id} ${item.room} ${item.type} ${item.caseId}`.toLowerCase()}">
      <td>${item.id}</td><td>${formatDateTime(item.dateTime)}</td><td>${item.room}</td><td>${item.caseId}</td><td>${item.type}</td><td><span class="badge ${item.status}">${hearingStatusLabel(item.status)}</span></td>
      <td>${actionCell("udienze", "data-edit-hearing", "data-delete-hearing", item.id)}</td>
    </tr>`).join("");
}

function initCasesPage() { initCrudPage("fascicoli", "case-form", "case-form-message", "fascicoli-body", "fascicoli-search", getCases, saveCases, renderCasesTable, (d) => ({ id: normalizeId(String(d.get("id") || ""), "#"), title: String(d.get("title") || "").trim(), state: String(d.get("state") || "open"), prosecutor: String(d.get("prosecutor") || "").trim(), priority: String(d.get("priority") || "Media"), createdAt: new Date().toISOString() }), (i) => ({ id: i.id, title: i.title, state: i.state, prosecutor: i.prosecutor, priority: i.priority }), "data-edit-case", "data-delete-case", "Fascicolo"); }
function initWarrantsPage() { initCrudPage("mandati", "warrant-form", "warrant-form-message", "mandati-body", "mandati-search", getWarrants, saveWarrants, renderWarrantsTable, (d) => ({ id: normalizeId(String(d.get("id") || "").toUpperCase()), caseId: normalizeId(String(d.get("caseId") || ""), "#"), type: String(d.get("type") || "").trim(), signer: String(d.get("signer") || "").trim(), status: String(d.get("status") || "active"), createdAt: new Date().toISOString() }), (i) => ({ id: i.id, caseId: i.caseId, type: i.type, signer: i.signer, status: i.status }), "data-edit-warrant", "data-delete-warrant", "Mandato"); }
function initSentencesPage() { initCrudPage("sentenze", "sentence-form", "sentence-form-message", "sentenze-body", "sentenze-search", getSentences, saveSentences, renderSentencesTable, (d) => ({ id: normalizeId(String(d.get("id") || "").toUpperCase()), caseId: normalizeId(String(d.get("caseId") || ""), "#"), outcome: String(d.get("outcome") || "").trim(), judge: String(d.get("judge") || "").trim(), penalty: String(d.get("penalty") || "").trim(), status: String(d.get("status") || "issued"), createdAt: new Date().toISOString() }), (i) => ({ id: i.id, caseId: i.caseId, outcome: i.outcome, judge: i.judge, penalty: i.penalty, status: i.status }), "data-edit-sentence", "data-delete-sentence", "Sentenza"); }
function initDepartmentsPage() { initCrudPage("reparti", "department-form", "department-form-message", "reparti-body", "reparti-search", getDepartments, saveDepartments, renderDepartmentsTable, (d) => ({ id: normalizeId(String(d.get("id") || "").toUpperCase()), name: String(d.get("name") || "").trim(), chief: String(d.get("chief") || "").trim(), activeCases: Number(String(d.get("activeCases") || "0")), status: String(d.get("status") || "operational"), createdAt: new Date().toISOString() }), (i) => ({ id: i.id, name: i.name, chief: i.chief, activeCases: i.activeCases, status: i.status }), "data-edit-department", "data-delete-department", "Reparto"); }
function initPeoplePage() { initCrudPage("persone", "person-form", "person-form-message", "persone-body", "persone-search", getPeople, savePeople, renderPeopleTable, (d) => ({ id: normalizeId(String(d.get("id") || "").toUpperCase()), name: String(d.get("name") || "").trim(), role: String(d.get("role") || "").trim(), caseId: normalizeId(String(d.get("caseId") || ""), "#"), createdAt: new Date().toISOString() }), (i) => ({ id: i.id, name: i.name, role: i.role, caseId: i.caseId }), "data-edit-person", "data-delete-person", "Soggetto"); }
function initHearingsPage() { initCrudPage("udienze", "hearing-form", "hearing-form-message", "udienze-body", "udienze-search", getHearings, saveHearings, renderHearingsTable, (d) => ({ id: normalizeId(String(d.get("id") || "").toUpperCase()), dateTime: String(d.get("dateTime") || "").trim(), room: String(d.get("room") || "").trim(), caseId: normalizeId(String(d.get("caseId") || ""), "#"), type: String(d.get("type") || "").trim(), status: String(d.get("status") || "open"), createdAt: new Date().toISOString() }), (i) => ({ id: i.id, dateTime: i.dateTime, room: i.room, caseId: i.caseId, type: i.type, status: i.status }), "data-edit-hearing", "data-delete-hearing", "Udienza"); }

function initCrudPage(section, formId, messageId, bodyId, searchId, getter, saver, renderer, extractor, editValues, editAttr, deleteAttr, entityName) {
  renderer(getter());
